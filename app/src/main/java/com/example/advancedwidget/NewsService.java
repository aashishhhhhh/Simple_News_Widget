...
public class NewsService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (ApplicationClass.connectionAvailable(getApplicationContext())){
            RemoteViews remoteViews = new RemoteViews("com.example.advancedwidget",R.layout.new_app_widget);
            remoteViews.setTextViewText(R.id.tvTitle,"Busy Rerieving data.......");
            remoteViews.setTextViewText(R.id.tvDesc,"Busy Rerieving data.......");

            AppWidgetManager.getInstance(getApplicationContext()).updateAppWidget(intent.getIntExtra("appWidgetId",0),remoteViews);

            new GetStoriesInBackground().execute();
        }
        else {
            Toast.makeText(getApplicationContext(),"Please make sure your connect to the internet",Toast.LENGTH_SHORT).show();
            stopSelf();
        }
    ...
    public class GetStoriesInBackground extends AsyncTask<Integer, Integer, String>
    {
        ...
        protected void onPostExecute(String result) {
            ...
            RemoteViews remoteViews = new RemoteViews("com.example.advancedwidget",R.layout.new_app_widget);
            remoteViews.setTextViewText(R.id.tvTitle,titles.get(randomValue));
            remoteViews.setTextViewText(R.id.tvDesc,descriptions.get(randomValue));
            ...
            remoteViews.setOnClickPendingIntent(R.id.tvDesc,pi);
            ...
            remoteViews.setOnClickPendingIntent(R.id.ivSync,pendingIntentSync);
            AppWidgetManager.getInstance(getApplicationContext()).updateAppWidget(intent.getIntExtra("appWidgetId",0),remoteViews);
